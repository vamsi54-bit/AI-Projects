    ...
    <main className="cinema-app">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {status === "live" ? `${results.length} face${results.length === 1 ? "" : "s"} · ${latency} ms` : "Waiting for camera"}
            </p>
          </div>
      ...
      <div className="control-dock glass-panel">
        {status === "live" || status === "loading" ? (
          <button type="button" onClick={stopCamera} className="primary-control stop" aria-label="Stop live detection">
